...
class Computer(program: List<BigInteger>) {
    ...
    fun run(inputList: List<BigInteger> = emptyList()): BigInteger? {
        val inputIterator = inputList.iterator()
        ...
        while (ip < mem.size) {
            when (getOpcode()) {
                ...
                OpCode.INPUT -> {
                    val inputValue = when {
                        inputIterator.hasNext() -> inputIterator.next()
                        inputStream != null -> inputStream!!.invoke()
                        else -> return null
                    }
